...
func DisableUser(ctx *gin.Context, phone string, err error) error {
	...
	log := model.LoginLog{}
	list, _, _ := log.Page(ctx, 1, int(consts.LoginLimit.PasswordLimit), nil, func(db *gorm.DB) *gorm.DB {
		return db.Where("username = ?", phone).Order("created_at desc")
	})
	if len(list) < int(consts.LoginLimit.PasswordLimit) {
		return nil
	}
	// 存在登陆成功则跳过
